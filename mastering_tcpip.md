...
## 7.1 経路制御とは
### 7.1.1 IPアドレスと経路制御
- ルーターは、受け取ったパケットの宛先IPアドレスと経路制御表を比較して、次に送信すべきルーターを決定する。

### 7.1.2 スタティックルーティングとダイナミックルーティング
- スタティックルーティング
  - ルーターやホストに固定的に経路情報を設定する方法
  - 通常は人の手で行うので、管理者に大きな負担がかかる。（ネットワークが追加された場合、すべてのルーターに経路情報の設定を追加する必要がある。）
- ダイナミックルーティング
  - ルーティングプロトコルを動作させ、自動的に経路情報を設定する
  - 管理者は、ルーティングプロトコルの設定を行わなければならない。設定の複雑さはルーティングプロトコルの種類によって異なる。
  - ルーターの数が多い場合には、管理の手間はスタティックルーティングよりは減る。
  - 経路制御に必要な情報を交換するために、ルーター同士が定期的にメッセージのやりとりを行う。このため、ネットワークにはある程度負荷がかかることになる。

### 7.1.3 ダイナミックルーティングの基礎
- ダイナミックルーティングは、隣り合うルーター間で自分が知っているネットワークの接続情報を教え合うことによって行われる。情報がバケツリレー式に転送され、ネットワークの隅々まで伝わると経路制御表が完成し、IPパケットを正しく転送できるようになる。

## 7.2 経路を制御する範囲
- 経路を制御する範囲によってIGPとEGPという2種類のルーティングプロトコルが使われる。

### 7.2.1 インターネットにはさまざまな組織が接続されている

### 7.2.2 自律システムとルーティングプロトコル
- 経路制御に関するルールを決めて、それをもとに運用する範囲を自律システムや、経路制御ドメインという。自律システムの具体例は、地域ネットワークやISP
- 自律システムの内部でダイナミックルーティングに利用されるプロトコルがIGP。自律システム間の経路制御に利用されるのがEGP

### 7.2.3 EGPとIGP
- EGPによって地域ネットワークやプロバイダ間の経路制御が行われ、IGPによってその地域ネットワークやプロバイダ内部のどのホストなのかが識別される。
- IGPでは、RIPやRIP2、OSPFなどのプロトコルが利用される。EGPでは、BGPが利用される。

## 7.3 経路制御アルゴリズム
- 経路制御アルゴリズムで代表的なものは、距離ベクトル型とリンク状態型の2種類。

### 7.3.1 距離ベクトル型
- 距離（メトリック：　通過するルーター数）と方向によって目的のネットワークやホストの位置を決定する方法
- ネットワークの構造が複雑になると、経路制御情報が安定するまでに時間がかかる、経路にループが生じやすくなる、といった問題がある

### 7.3.2 リンク状態型
- ルーターがネットワーク全体の接続状態を理解して経路制御表を作成する方法。すべてのルーターが同じ情報を持つことになる。
- 各ルーターが経路制御情報を他のルーターと素早く同期させることに専念すれば、経路制御を安定させることができる。
- 代償として、ネットワークトポロジーから経路制御表を求める計算はかなり複雑で、多くのコンピューターリソースを必要とする。

### 7.3.3 主なルーティングプロトコル
- EGPはCIDRに対応していないため、現在のインターネットの対外接続用プロトコルでは使われていない

## 7.4 RIP
- 距離ベクトル型のルーティングプロトコルで、LANで広く使用されている。

### 7.4.1 経路制御情報をブロードキャストする
- RIP は経路制御情報を定期的にネットワーク上にブロードキャストする。経路制御情報の送信間隔は30秒周期。
- ブロードキャストする情報は、特定のネットワークの自ルーターからの距離の情報？と思われる。情報伝達されたルーターは、知った情報に距離を1足してからブロードキャストする。これを繰り返して少しずつ情報が伝わっていく。
- 経路制御情報が一定時間来なくなったら接続が切れたと判断する。

### 7.4.2 距離ベクトルにより経路を決定
- RIPは距離ベクトルにより経路を決定する。距離の単位は「ホップ数（＝通過するルーター数）」。

### 7.4.3 サブネットマスクを利用した場合のRIPの処理
- ?

### 7.4.4 RIPで経路が変更されるときの処理
- 経路情報送信時に通信障害が起きた場合に、過去に伝えた情報を逆に教えられ、それをお互いに伝え合ってしまう無限カウントという問題が生じる。この問題を解決するために2つの方法が取られる。
  - 距離16を通信不能とする
  - 経路情報を教えられたインタフェースには教えられた経路情報を流さない。これをスプリットホライズンと呼ぶ。
- これでも解決できないネットワークがある。これをできるだけ解決するために、ポイズンリバースとトリガードアップデートという2つの方法が利用される。
- OSPF
...
### 7.5 OSPF
- OSPFはリンク状態型のルーティングプロトコル。リンク状態型のため、ループのあるネットワークでも安定した経路制御が可能。
- OSPFはサブネットマスクをサポートしている。また、トラフィックを軽減させるために、エリアの概念が導入された。

### 7.5.1 OSPFはリンク状態型のルーティングプロトコル
- ルーター間で、ネットワークのリンク状態を交換し、ネットワークのトポロジーの情報を作成する。そして、そのトポロジー情報をもとにして経路制御表を作成する。
- OSPFでは各リンクに重みをつけることができ、この重みが小さくなるように経路が選択される。OSPFではこの重みのことをコストと呼ぶ。コストの合計値が小さくなるように経路制御される。

### 7.5.2 OSPFの基礎知識
- 隣接ルーター
- 指名ルーター
- OSPFでは、役割ごとに5種類のパケットを用意している。こうすることでトラフィックを軽減させながらよりスピーディーに経路を更新できるようになっている。

### 7.5.3 OSPFの動作の概要
- HELLOパケットでルーター間の接続の確認を行う。
- 接続状態に変化があった場合には、リンク状態更新パケットを送信して、他のルーターにネットワークの状態変化を伝える。
- リンク状態更新パケットで伝える情報は、ネットワークLSAとルーターLSA。
  - ネットワークLSAは、そのネットワークにどのルーターが接続されているかを表す。
  - ルーターLSAは、そのルーターにどのネットワークが接続されているかを表す。
- これらの情報がOSPFによって送られてくると、各ルーターはネットワークの構造を表すリンク状態データベースを作成する。このデータベースをもとにして経路制御表を作成する。

### 7.5.4 階層化されたエリアに分けてきめ細かく管理
- OSPFでは経路制御情報の計算の負荷を軽減するために、エリアという概念が取り入れられている。
- エリアとは、ネットワーク同士やホスト同士をまとめてグループ化したもの。
- 各AS（自律システム）には、必ず1つのバックボーンエリアが必要。また、各エリアは必ずバックボーンエリアに接続されていなければならない。
- エリア境界ルーター、内部ルーター、バックボーンルーター、AS境界ルーター
- 各エリア内のルーターは、そのエリア内のトポロジーのデータベースを持っている。ルーターはエリア内部のリンク状態だけを知っており、その情報だけから経路制御表を計算すれば良い。この仕組みにより、処理の負荷を軽くすることができる。

## 7.6 BGP
- BGPは組織間を接続するときに利用されるプロトコルで、EGPに分類される。具体的には各ISP間の接続部分などに利用される。

### 7.6.1 BGPとAS番号
- ISPや地域ネットワークなどの組織を束ねるネットワーク集団を1つの自律システム（AS）として取り扱う。それぞれの自律システムごとにAS番号が割り当てられる。BGPでは、このAS番号を使って経路制御が行われる。
- ASが他のASと接続するときは、契約を結んで接続する事になる。

### 7.6.2 BGPは経路ベクトル
- BGPにより経路制御情報を交換するルーターをBGPスピーカーという。BGPスピーカーは、情報を交換するすべてのASと対等にBGPのコネクションを確立する。
- BGPでは、目的とするネットワークアドレスにパケットを送った場合に、そこに到達するまでに通過するAS番号のリストが作られる。これをAS経路リストという。
- BGPで経路の選択に使われるメトリックの単位はAS。基本的には経由するASの数が少ないルートを選択する。
- BGPのように、通過する経路のリストで経路制御を行うプロトコルを、経路ベクトル型という。

## 7.7 MPLS
- IPパケットの転送には、ルーティングだけでなくラベルスイッチングという技術も利用される。ラベルスイッチングでは、IPパケットに「ラベル」という別の値を設定し、そのラベルに基づいて転送する。ラベルスイッチングの代表がMPLS。
- MPLSは、イーサネットなどの下位層とIP層との間の階層で機能するプロトコルと考えることができる。
- ラベルに基づいた転送は、通常のルーターでは処理できないため、MPLSはインターネット全体で利用できる技術ではない。

### 7.7.1 MPLSネットワークの動作
- LSR, LER, シムヘッダ, LSP

### 7.7.2 MPLSの利点
1. 転送処理の高速化　処理が単純になり転送処理のハードウェア化による高速化が可能
2. ラベルを利用して仮想的なパスを張り、その上でIPなどのパケットを使った通信ができる点

# 第8章 アプリケーションプロトコル
## 8.1 アプリケーションプロトコルの概要
- アプリケーションごとの特有の通信処理を行うのがアプリケーションプロトコル
- ネットワークアプリケーションの機能を実現するためには、アプリケーション間で通信するときの取り決め、すなわちアプリケーションプロトコルが必要である。

## 8.2 遠隔ログイン（TELNETとSSH）
- 遠隔ログインは、TSS（Time Sharing System）のような環境を実現するアプリケーション

### 8.2.1 TELNET
- TCPのコネクションを1つ利用。この通信路を通して相手のコンピュータにコマンドが文字列として送信され、相手のコンピュータで実行される。
- TELNETのサービスは、2つの基本サービスに分けることができる。ネットワーク仮想端末の機能と、オプションのやりとりをする機能。

#### オプション
- TELNETでは、ユーザーが入力した文字以外にオプションをやり取りする機能が用意されている。
- 行モードと透過モードというモードの設定も、オプション機能を利用して設定される。

#### TELNETクライアント
- TELNETクライアントは、デフォルトポート23番以外のポートで待っているアプリケーションを実行することもできる。
- 次の2つの入力は同等。そのため、TCP/IPのアプリケーション開発でのデバッグに利用されることもある。
```
ftp ホスト名
telent ホスト名 21
```
...
## 8.3 ファイル転送（FTP）
- FTPは異なるコンピュータ間でファイルを転送するときに使われるプロトコル
#### FTPの仕組みの概要
- FTPでは2つのTCPコネクションが利用される。1つは制御用で、もう1つはデータ（ファイル）の転送用。
#### ASCII文字列によるやり取り
- FTPでは、要求コマンドに「RETR」などのASCII文字列が利用される。それに対する応答は「200」など3文字の数字をASCII文字列で表したものが利用される。
- ASCII文字列型のプロトコルでは改行が重要な意味を持つ。

## 8.4 電子メール（E-Mail)
- 電子メールは世界中のどこにでも送ることができる。また、メーリングリストで一斉配信できるなどの利点がある。

### 8.4.1 電子メールの仕組み
- 電子メールサービスを提供するためのプロトコルがSMTP。SMTPはトランスポートプロトコルとしてTCPを使う。
- 初期はユーザー間でTCPでメールの送受信をしていたが、うまくいかなくなったのでメールサーバーを経由するようになった。受信者がメールサーバーから電子メールを受け取るPOPというプロトコルが標準化された。
- 電子メールの仕組みは、メールアドレス、データ形式、転送プロトコルの3要素から構成される。

### 8.4.2 メールアドレス
- メールアドレスは郵便でいえば住所と氏名に相当する。次のような構造
```
名前@住所
```
- 現在、電子メールの配送先の管理はDNSによって行われる。
- DNSには、メールアドレスと、そのメールアドレス宛のメールを送信すべきメールサーバーのドメイン名を登録できる。これをMXレコードという。 これにより、kusa.ac.jpで終わるメールアドレス宛のメールを、mailserver.kusa.ac.jpに転送するといったことができる。

### 8.4.3 MIME
- MIMEはOSI参照モデルでは第6層プレゼンテーション層に当たるもので、電子メールで転送できるデータ形式を拡張したもの。
- ヘッダと本文（データ）から構成される。
- マルチパート
- 「Content-Type」は、ヘッダに続く情報がどのような種流のデータなのかを示す。

### 8.4.4 SMTP
- SMTPは電子メールを配送するアプリケーションプロトコル。ポート番号25
- SMTPは1つのTCPコネクションを確立し、その上で制御や応答、データからなるメッセージの転送を行う。

### 8.4.5 POP
- POPは電子メールを受信するためのプロトコル
- メールは送信者からSMTPによって常時電源が入っているPOPサーバーまで到着する。クライアント（受信者側）はPOPによってPOPサーバーに保存された相手のメールを取り出す。

### 8.4.6 IMAP
- IMAPはPOPと同様に、電子メールなどのメッセージを受信するためのプロトコル。POPの場合は電子メールの管理をクライアント側で行うが、IMAPの場合はサーバー側で管理する。
- IMAPを使うとサーバー上に保持、保管されているメールメッセージを、あたかも自分の使うクライアントの記憶媒体のように扱うことができる。

## 8.5 WWW(World Wide Web)
### 8.5.1 インターネットブームの火付け役
- WWWはインターネット上の情報をハイパーテキスト形式で参照できる情報提供システム

### 8.5.2 WWWの基本概念
- 情報へのアクセス手段と位置の定義（URI）、情報の表現フォーマットの定義（HTML）、情報の転送などの操作の定義（HTTP）

### 8.5.3 URI
- スキーム：　URIが表す枠組み

### 8.5.4 HTML
- Webページを記述するための言語（データ形式）。HTMLは、WWWの共通のデータ表現プロトコルということができる。

#### XML
- WWWに置いてデータをファイルに保存したり、アプリケーション間でやり取りしたりする形式としてXMLが利用される。

### 8.5.5 HTTP
- HTTP1.1から1つのTCPコネクションで複数のコマンド、応答ができるようになった。HTTP1.0までは1つのコマンド、応答をやり取りするたびにTCPコネクションを確立、切断していた。

### 8.5.6 JavaScript、CGI、クッキー
- JavaScript
- CGI
  - Webサーバーが外部プログラムを呼び出すサーバーサイドアプリケーションの仕組み
- クッキー

## 8.6 ネットワーク管理（SNMP）
### 8.6.1 SNMP
- TCP/IPのネットワーク管理ではSNMPが必要な情報の取得などを行うために利用される。
- 管理する側をマネージャ、管理される側をエージェントと呼ぶ。
- SNMPv3 「メッセージ処理」、「セキュリティ」、「アクセスコントロール」を分けて考え、それぞれに必要な仕組みを選択できるようにしている。
- 通常は、参照要求・応答によって定期的に機器の動作をチェックしたり、設定要求によって機器設定を変更したりする。SNMPでの処理は機器へのデータの書き込みと読み込みという2つに集約できる。

### 8.6.2 MIB
- SNMPでやり取りされる情報が、MIB。MIBはツリー型の構造を持ったデータベースで、それぞれの項目に番号がつけられている。

### 8.6.3 RMON
- Remote Monitoring MIBの略称
- 接続されるネットワークの回線（線）を監視するパラメータ群からなる
- ネットワークの利用率や回線に負荷をかけているホストやプロトコルを特定するための情報を得ることができる

## 8.7 その他のアプリケーションプロトコル
### 8.7.1 マルチメディア通信を実現する技術（H.323、SIP、RTP）
#### H.323
#### SIP
- OSI参照モデルで言えば、セッション層に相当する。
- SIP自体は端末間でメッセージのやり取りにより呼制御を行うだけで、マルチメディアデータ自体の転送は行わない。端末間だけでなく、サーバーを介して転送することもできる。
#### RTP
- UDPでリアルタイムなマルチメディア通信を実現するためには、パケットの順番を表すシーケンス番号を付けたり、パケットの送信時刻を管理したりする必要がある。これを行うのがRTP。
#### デジタル圧縮技術
- MPEG MP3もMPEGの規格
- デジタル圧縮技術は、データの形式を規定しているため、OSI参照モデルのプレゼンテーション層に相当すると言える。

### 8.7.2 P2P(Peer To Peer)
- ネットワーク上に展開する各端末やホストがサーバーなどを介さずに1対1で直接接続し通信を行う形態がP2P。
- P2Pでは、各ホストがクライアントとサーバーの両方の機能をもち、対等な関係でサービスを相互に提供し合う。

### 8.7.3 LDAP
- LDAPは、ディレクトリサービスにアクセスするためのプロトコル。ディレクトリサービスはネットワーク上の資源の管理サービスと考えて良い。

# 第9章 セキュリティ
## 9.1 TCP/IPとセキュリティ
- 便利なインターネットを安全に利用するためには、利便性の一部を犠牲にしてセキュリティを確保するしかない。「利便性」と「安全性」という相反する事柄を両立させるために、多くの技術革新が進んでいる。

## 9.2 セキュリティの構成要素
- セキュリティの基本中の基本は、事前対策。

### 9.2.1 ファイアウォール
- 組織内のネットワークとインターネットを接続するときには、組織内部のネットワークを不正アクセスから守るためにファイアウォールを設置する必要がある。
- ファイアウォールにはいくつもの種類があるが、基本的な考え方は、「危険にさらすのは特定のホストやルーターのみに限定し、それらに集中してセキュリティをかける」ということ。
- 規定されたパケットのみを通過させるパケットフィルタリングタイプや、アプリケーションを介在させ不正な接続を遮断するアプリケーションゲートウェイタイプなどがある。

### 9.2.2 IDS(侵入検知システム)
- ファイアウォールは基本的にポリシーと合致した通信であれば通過させるので、悪意のある通信であったとしても判断することはできずに通過させてしまう。このような通信や、一旦内部などに侵入して不正アクセスを行う通信を見つけ、セキュリティの管理者などに通知をするのがIDSと言われるもの

### 9.2.3 アンチウイルス／パーソナルファイアウォール
- アンチウイルス／パーソナルファイアウォールは、ユーザーが利用するコンピュータや、サーバーなどで動作するソフトウェア。そのコンピュータを出入りするパケットやデータ、ファイルなどを監視して、不正な処理やウイルスの侵入などを防ぐ。
- アンチウイルス／パーソナルファイアウォールは、種々の脅威からOS、つまりクライアントPCを防御するためのものである。

## 9.3 暗号化技術の基礎
- 暗号化技術はOSI参照モデルの階層ごとに存在する。暗号化が行われる階層より上位層が暗号化により盗聴から保護される。
  - アプリケーション：　SSHなど遠隔ログイン、S/MIMEなど暗号化メール
  - セッション、トランスポート：　SSL/TLS、SOCKS V5の暗号化
  - ネットワーク：　IPsec
  - データリンク：　Ethernet、WANの暗号化装置など

### 9.3.1 共通鍵暗号方式と公開鍵暗号方式
- 暗号化と復号に同じ鍵を使うのが共通鍵暗号方式。暗号化、復号に一対の別々の鍵（公開鍵と秘密鍵）を使うのが公開鍵暗号方式。
- 長いメッセージを暗号化する場合には、秘密鍵暗号方式と共通鍵暗号方式を組み合わせて利用する。

### 9.3.2 認証技術
- ある情報を持っていることによる認証：　パスワードや暗証番号を利用
- あるものを持っていることによる認証：　IDカード、鍵、電子証明書、電話番号などを利用
- ある特徴を持っていることによる認証：　指紋や目の瞳孔といった特徴を利用
- 一般には、必要とされる認証レベルと費用対効果の観点から、上の3つを組み合わせて利用することが多い。

## 9.4 セキュリティのためのプロトコル
### 9.4.1 IPsecとVPN
- インターネットを利用した仮想的な私的ネットワークがVPN
- VPN構築に最も一般的に利用されているのがIPsec
- IPsecでは、IPヘッダの後ろに、「暗号ヘッダ」や「認証ヘッダ」をつける。そして、そのヘッダ以降のデータを暗号化して、盗聴されても解読できないようにする。
- トランスポートモード、トンネルモード

### 9.4.2 TLS/SSLとHTTPS
- Webでは、TLS/SSLという仕組みを使ってHTTP通信の暗号化が行われる。この通信をHTTPSと呼ぶ。HTTPSでは、共通鍵暗号方式で暗号化処理が行われる。この共通鍵を送信するときには公開鍵暗号方式が利用される。
- 手順
  1. クライアントが、WebサーバーにHTTPSでアクセスする。
  2. Webサーバーは、「Webサーバーの公開鍵」と「電子証明書」をクライアントに送信する。電子証明書は、認証局の秘密鍵で暗号化されている。
  3. クライアントは、送られてきた電子証明書を、あらかじめブラウザに組み込まれている認証局の公開鍵で復号し、正規の認証局が発行したものであると確認できたら、正しいWebサーバーと通信できていると判断して、次の手順に移る。
  4. クライアントは、共通鍵暗号方式の共通鍵を作成し、Webサーバーの公開鍵を使って暗号化して送信する。
  5. Webサーバーは、クライアントから送られてきた暗号化された共通鍵を、自身の持つ秘密鍵で復号して、共通鍵を知る。
  6. 共通鍵を交換できたので、それを使って共通鍵暗号方式による暗号化通信を開始する。

### 9.4.3 IEEE802.1X
- IEEE802.1X は、認められた機器のみがネットワークにアクセスできるように認証する仕組み。無線アクセスや構内LANでよく使われる。
- 手順
  1. ユーザー端末が無線LANアクセスポイントへ接続要求
  2. アクセスポイントは、認証サーバーのあるVLANへの一時的なIPアドレスなどをユーザー端末に付与する。これにより、ユーザー端末は認証サーバーとしか通信できないネットワークに接続される。
  3. 接続された後、認証に必要なIDとパスワードを要求されるので、入力する。
  4. その情報が認証サーバーで確認され、利用者が利用可能なネットワーク情報が、端末とアクセスポイントに通知される。
  5. アクセスポイントは、ユーザー端末が利用可能なVLANに接続の切り替えを行う。
  6. その後、ユーザー端末がIPアドレスをリセットして再設定を行う。
  7. ユーザー端末が、利用可能なネットワークへ接続できるようになる。

# 付録.3 物理層
## 付.3.1 物理層についての基礎知識
- 物理層の規格では、ビットと信号の変換の規則、ケーブルの構造や品質、コネクタの形状などを規定している。

## 付.3.2 0と1の符号化
- 送信側では「0」と「1」のデータを電圧の変化や光の点滅に変換し、受信側では電圧の変化や光の点滅を「0」と「1」のデータに戻す。
- 符号化方式
  - マンチェスター、NRZ、NRZI、MLT-3　など

# 付録.4 コンピュータを結ぶ通信媒体についての基礎知識
## 付.4.1 同軸ケーブル

## 付.4.2 ツイストペアケーブル（より対線）
- イーサネット（10BASE-T、100BASE-TX、1000BASE-T）の媒体として最もよく使われる。

#### 信号の伝送方式
- 伝送方法には2種類ある
1. RS-232Cを使用した通信に代表される方式
2. RS-422に代表される方式
  - グラウンド信号を使用せず、伝送するビット列に対応する信号（プラス側信号）と、それと正反対の信号（マイナス側）を1対にして送信する。
  - 他の通信に影響を与える可能性が少ない
  - 外部からの電気的影響（ノイズ）に対する耐性が高い
  - ツイストペアケーブルでのイーサネットはこちらに当たる

#### ツイストペアケーブルの種類
- ツイストペアケーブルには、STPとUTPの2種類がある。
- ネットワークの種類により、使用するツイストペアケーブルの種類は変わる。

#### ツイストペアケーブルのペアの組み合わせ

## 付.4.3 光ファイバーケーブル
- 光ファイバーケーブルは、数km離れた遠隔地を接続する場合や、ノイズなどの電磁波障害からネットワークを保護する場合、そしてより高速に伝送する場合などに使用される。
- マルチモードタイプ、シングルモードタイプ、WDM

## 付.4.4 無線
- 無線は空間中を飛び交う電磁波を利用する。
- ネットワークの無線通信でよく利用されるのは赤外線とマイクロ波
- 無線LANなどでは2.4GHz帯の極超短波と呼ばれる周波数帯を使用する。
