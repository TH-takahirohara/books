...
- 1つのURLが異なるリソースに対応しなければならないことがよくあります。例えば、複数の言語でコンテンツを提供したいWebサイトがあるとします。Joe's Hardwareのように、フランス語と英語の両方を話すユーザーがいる場合、ウェブサイトを両方の言語で提供したいと思うかもしれません。しかし、このような場合、Joe'sの顧客が「http://www.joes-hardware.com」とリクエストしたとき、サーバーはどちらのバージョンを送信すべきでしょうか? フランス語でしょうか、それとも英語でしょうか。
- 理想的には、サーバーは、英語を話す人には英語版を、フランス語を話す人にはフランス語版を送信し、ユーザーは、Joe's Hardwareのホームページにアクセスして、自分が話す言語でコンテンツを得ることができます。幸いなことに、HTTPには、クライアントとサーバがそのような判断をするためのコンテンツネゴシエーション手法が用意されています。この方法を使えば、1つのURLが異なるリソース（例えば、同じWebページのフランス語版と英語版）に対応することができます。これらの異なるバージョンをバリアント（variants）と呼びます。
- また、サーバーは、特定のURLに対してどのようなコンテンツをクライアントに送るのが最適か、さまざまな判断を下すことができます。例えば、HTMLページを携帯端末用のWMLページに変換することもできます。このような動的なコンテンツ変換をトランスコーディングといいます。トランスコーディングは、HTTPクライアントとサーバー間のコンテンツネゴシエーションに応じて行われます。
- この章では、コンテンツネゴシエーションと、ウェブアプリケーションがどのようにコンテンツネゴシエーションの義務を果たすかについて説明します。
...
- 第5部では、ウェブコンテンツを公開・配信するための技術について解説しています。
  - 第18章では、最新のウェブホスティング環境におけるサーバーの配置方法、バーチャルウェブホスティングのためのHTTPサポート、地理的に離れたサーバー間でのコンテンツの複製方法などについて説明しています。
  - 第19章では、ウェブコンテンツを作成し、ウェブサーバーにインストールするための技術について説明しています。
  - 第20章では、受信したウェブトラフィックを複数のサーバーに分散させるためのツールやテクニックについて説明しています。
  - 第21章では、ログのフォーマットとよくある質問について説明しています。

# Chapter 18. Web Hosting
- パブリックウェブサーバーにリソースを置くと、インターネットコミュニティにそのリソースを提供することができます。これらのリソースは、テキストファイルや画像のように単純なものから、リアルタイムのドライブマップやEコマースのショッピングゲートウェイのように複雑なものまであります。さまざまな組織が所有するこの豊富な種類のリソースを、便利にWebサイトに公開し、適正な価格で優れたパフォーマンスを提供するWebサーバーに置くことが重要です。
- このように、コンテンツリソースの保管、仲介、管理を一括して行う業務をウェブホスティングといいます。ホスティングは、ウェブサーバーの主要な機能のひとつです。コンテンツを保持し、提供し、アクセスを記録し、管理するためにはサーバーが必要です。必要なハードウェアやソフトウェアを自分で管理したくない場合は、ホスティングサービス（ホスター）を利用します。ホスティングサービスは、サーバーやウェブサイトの管理サービスを提供するもので、さまざまなレベルのセキュリティやレポート機能、使いやすさを提供します。ホスターは、コスト効率、信頼性、パフォーマンスに優れた高性能のウェブサーバーにウェブサイトをプールしています。
- 本章では、Web ホスティングサービスの最も重要な機能と、それらと HTTP アプリケーションとの関係について説明します。本章では、特に以下の内容について説明します。
  - 同一サーバ上で異なるWebサイトを「バーチャルホスト」する方法と、それによるHTTPへの影響
  - 大量のトラフィックが発生した場合にウェブサイトの信頼性を高める方法
  - Webサイトの読み込みを高速化する方法

## 18.1 Hosting Services
- ワールドワイドウェブの黎明期には、個々の組織が独自にコンピュータハードウェアを購入し、コンピュータルームを建設し、ネットワーク接続を行い、ウェブサーバソフトウェアを管理していました。
- Webが急速に普及すると、誰もがWebサイトを欲しがるようになりましたが、空調の効いたサーバールームを作ったり、ドメイン名を登録したり、ネットワークの帯域幅を購入したりするスキルや時間のある人はほとんどいませんでした。このような状況を打開するために、プロが管理するウェブホスティングサービスを提供する新しいビジネスが数多く登場しました。物理的な設備管理（スペース、空調、配線などの提供）から、お客様がコンテンツを提供するだけのフルサービスのウェブホスティングまで、さまざまなレベルのサービスがあります。
- 本章では、ウェブサーバーが提供するサービスについて説明します。Webサイトの機能、例えば、さまざまな言語をサポートする機能や、安全な電子商取引を行う機能などは、ホスティングWebサーバーがどのような機能をサポートしているかによって決まります。

### 18.1.1 A Simple Example: Dedicated Hosting
- 「Joe's Hardware Online」と「Mary's Antique Auction」の2社が、かなりボリュームのあるWebサイトを望んでいるとします。アイリーンのISPは、ジョーとメアリーが自分でサーバを購入してサーバソフトウェアを維持する代わりに、ジョーとメアリーにリースできる同一の高性能ウェブサーバを何台も持っている。
- 図18-1では、JoeとMaryの二人が、IreneのISPが提供する専用のWebホスティングサービスを契約しています。Joeは、IreneのISPが購入して保守している専用Webサーバを借ります。Maryは、Irene's ISPから別の専用サーバを取得します。アイリーンのISPは、サーバーハードウェアを大量に購入することができ、信頼性が高く、実績があり、低コストのハードウェアを選択することができます。「Joe's Hardware Online」または「Mary's Antique Auction」のいずれかが人気を博した場合、Irene's ISPはJoeまたはMaryにすぐに追加のサーバーを提供することができます。
- この例では、ブラウザは www.joes-hardware.com に対する HTTP リクエストを Joe のサーバーの IP アドレスに送り、 www.marys-antiques.com に対するリクエストを Mary のサーバーの（異なる）IP アドレスに送ります。

## 18.2 Virtual Hosting
- 多くのお客様は、ウェブ上で存在感を示したいと考えていますが、トラフィックの多いウェブサイトを持っていません。このような方々にとって、専用のウェブサーバーを提供することは無駄なことかもしれません。なぜなら、ほとんどアイドル状態のサーバーをリースするために、毎月何百ドルも支払っているのですから！
- 多くのウェブホスティング事業者は、1台のコンピュータを複数のお客様で共有することで、低価格のウェブホスティングサービスを提供しています。これは、シェアードホスティングまたはバーチャルホスティングと呼ばれています。それぞれのWebサイトは別のサーバーでホストされているように見えますが、実際には同じ物理サーバーでホストされています。エンドユーザーから見れば、バーチャルホストのWebサイトは、別々の専用サーバーでホストされているサイトと見分けがつかないはずです。
- コスト効率、スペース、管理などの理由から、バーチャル・ホスティング会社は、数十、数百、数千のウェブサイトを同じサーバーでホストしたいと考えていますが、必ずしも1,000のウェブサイトを1台のPCから提供することを意味しているわけではありません。ホスティング会社は、複製された（replicated）サーバーのバンク（サーバーファームと呼ばれる）を作り、負荷をサーバーファームに分散させることができます。ファーム内の各サーバは他のサーバのクローンであり、多くの仮想Webサイトをホストしているため、管理が非常に簡単です。(サーバーファームについては、第20章で詳しく説明します。）
- ジョーとメアリーがビジネスを始めたとき、彼らは、トラフィックレベルが専用サーバーに見合うものになるまで、費用を節約するためにバーチャル・ホスティングを選択したかもしれません（図18-2参照）。

### 18.2.1 Virtual Server Request Lacks Host Information
- 残念ながら、HTTP/1.0には、バーチャルホスターを悩ませる設計上の欠陥があります。HTTP/1.0の仕様では、共有ウェブサーバーが、自分がホストしている仮想ウェブサイトのどれがアクセスされているのかを識別する手段が与えられていませんでした。
- HTTP/1.0のリクエストでは、リクエストメッセージの中でURLのパス成分のみが送信されることを思い出してください。http://www.joes-hardware.com/index.html を取得しようとすると、ブラウザはサーバー www.joes-hardware.com に接続しますが、HTTP/1.0 リクエストでは「GET /index.html」となり、ホスト名についてはそれ以上言及されません。サーバが仮想的に複数のサイトをホストしている場合、どの仮想的なWebサイトにアクセスしているのかを把握するには、これだけでは十分な情報とは言えません。例えば、図18-3の場合。
  - クライアントAが http://www.joes-hardware.com/index.html にアクセスしようとすると、「GET /index.html」というリクエストが共有ウェブサーバーに送信されます。
  - クライアントBが http://www.marys-antiques.com/index.html にアクセスしようとすると、同じリクエスト「GET /index.html」が共有ウェブサーバーに送信されます。
- Webサーバーにとっては、どのWebサイトにアクセスしているのかを判断するのに十分な情報がありません。2つのリクエストは、（異なるWebサイトの）全く異なるドキュメントに対するものであるにもかかわらず、同じように見えます。問題は、Webサイトのホスト情報がリクエストから取り除かれていることです。
- 第6章で見たように、HTTPサロゲート（リバースプロキシ）やインターセプティングプロキシにもサイトを特定する情報が必要です。

### 18.2.2 Making Virtual Hosting Work
- このホスト情報の欠落は、オリジナルのHTTP仕様の見落としで、各ウェブサーバが正確に1つのウェブサイトをホストしていると誤って想定していました。HTTPの設計者は、仮想的にホストされている共有サーバーをサポートしていませんでした。このため、URL のホスト名情報は冗長であるとみなされて削除され、パス成分のみが送信されることになりました。
- 初期の仕様ではバーチャルホストに対応していなかったため、ウェブホスティング事業者は共有バーチャルホストに対応するための回避策や規約を策定する必要がありました。この問題は、すべてのHTTPリクエストメッセージに、パス成分だけではなく、完全なURLの送信を義務付けるだけで解決できたはずです。HTTP/1.1では、サーバーがHTTPメッセージのリクエスト行でフルURLを扱うことを要求していますが、すべてのレガシーアプリケーションがこの仕様にアップグレードされるまでには長い時間がかかるでしょう。それまでの間、4つのテクニックが登場しています。

  - URLパスによるバーチャルホスティング
    - URLに特別なパス成分を追加して、サーバーがサイトを判断できるようにすること。

  - ポート番号によるバーチャル・ホスティング
    - サイトごとに異なるポート番号を割り当てることで、リクエストを別々のウェブサーバーで処理します。Webサーバーの別のインスタンスで処理されるようになります。

  - IPアドレス型バーチャル・ホスティング
    - 異なる仮想サイトに異なるIPアドレスを割り当て、すべてのIPアドレスを1台のマシンにバインドします。これにより、WebサーバーはIPアドレスでサイト名を識別することができます。

  - ホストヘッダによるバーチャルホスティング
    - この問題を解決するために、多くのウェブホスティング事業者がHTTPの設計者に圧力をかけました。HTTP/1.0の拡張版とHTTP/1.1の正式版では、サイト名を伝えるHostリクエストヘッダを定義しています。Webサーバーは、Hostヘッダーから仮想サイトを特定することができます。
...
- より優れたアプローチ（一般的に使用されている）は、仮想IPアドレスです。ここでは、各仮想Webサイトが1つまたは複数の固有のIPアドレスを取得します。すべての仮想WebサイトのIPアドレスは、同じ共有サーバーに接続されています。サーバーは、HTTP接続の宛先IPアドレスを調べて、クライアントがどのWebサイトに接続していると考えているかを判断することができます。
- 例えば、www.joes-hardware.com に 209.172.34.3 という IP アドレスを割り当て、www.marys-antiques.com に 209.172.34.4 という IP アドレスを割り当て、両方の IP アドレスを同じ物理サーバマシンに結びつけたとします。Web サーバは、図 18-4 に示すように、宛先 IP アドレスを使用して、どの仮想サイトが要求されているかを識別することができます。
- バーチャルIPホスティングは有効ですが、特に大規模なホスターにとってはいくつかの問題があります。
  - コンピュータシステムでは通常、1台のマシンに割り当てられる仮想IPアドレスの数に制限があります。何百、何千ものバーチャルサイトを共有サーバーでホスティングしたいと考えているホスターは、運が悪いとしか言いようがありません。
  - IPアドレスは非常に希少なものです。多くのバーチャルサイトを持つホスターは、ホストされているWebサイトに十分な仮想IPアドレスを取得できない可能性があります。
  - IPアドレスの不足は、ホスターがサーバーを複製して容量を増やしている場合、さらに悪化します。負荷分散のアーキテクチャによっては、複製されたサーバごとに異なる仮想IPアドレスが必要となるため、必要なIPアドレスの数は複製されたサーバの数だけ増えることになります。
- バーチャルIPホスティングにはアドレス消費の問題がありますが、広く利用されています。

#### 18.2.2.4 Virtual hosting by Host header
- アドレスの過剰消費やバーチャルIPの制限を避けるため、同じIPアドレスをバーチャルサイト間で共有しつつ、サイトを見分けられるようにしたいと考えています。しかし、これまで見てきたように、ほとんどのブラウザはURLのパス成分だけをサーバに送信するため、重要なバーチャルホスト名の情報が失われてしまいます。
- この問題を解決するために、ブラウザとサーバの実装者はHTTPを拡張し、オリジナルのホスト名をサーバに提供するようにしました。しかし、ブラウザは単に完全なURLを送ることはできませんでした。それでは、パス成分だけを受け取ることを期待していた多くのサーバーが壊れてしまうからです。その代わり、ホスト名（とポート）はすべてのリクエストのHost extension headerに渡されます。
- 図18-5では、クライアントAとクライアントBの両方が、アクセスする元のホスト名を伝えるHostヘッダーを送信しています。サーバーは、/index.htmlのリクエストを受け取ると、Hostヘッダーを使用してどのリソースを使用するかを決定することができます。
- Hostヘッダは、HTTP/1.0のベンダー拡張版であるHTTP/1.0+で初めて導入されました。HTTP/1.1 に準拠するには、ホストヘッダが必要です。Host ヘッダはほとんどの最新ブラウザやサーバでサポートされていますが、サポートしていないクライアントやサーバ (およびロボット) もまだ少数ながら存在します。

### 18.2.3 HTTP/1.1 Host Headers
- Host ヘッダーは、RFC2068 で定義されている HTTP/1.1 のリクエストヘッダーです。仮想サーバーは一般的に使われるものなので、HTTP/1.1に準拠していなくても、ほとんどのHTTPクライアントはHostヘッダーを実装しています。

#### 18.2.3.1 Syntax and usage
- Hostヘッダーは、オリジナルのURLから取得した、リクエストされているリソースのインターネットホストとポート番号を指定します。
...
- 特に
  - Hostヘッダにポートが含まれていない場合、そのスキームのデフォルトポートが想定されます。
  - URLにIPアドレスが含まれている場合、Hostヘッダには同じアドレスが含まれている必要があります。
  - URL がホスト名を含む場合，Host ヘッダは同じ名前を含まなければならない。
  - URLにホスト名が含まれる場合、Hostヘッダには、URLのホスト名に相当するIPアドレスを含めるべきではありません。これは、単一のIPアドレスに複数の仮想サイトを重ねるバーチャルホストサーバが破損するためです。
  - URLにホスト名が含まれている場合、Hostヘッダにそのホスト名の別のエイリアスを含めてはいけません。これもバーチャルホストサーバを破壊してしまうからです。
  - クライアントが明示的なプロキシサーバを使用している場合、クライアントはプロキシサーバではなくオリジンサーバの名前とポートをHostヘッダに含めなければなりません。過去にいくつかのWebクライアントで、クライアントのプロキシ設定が有効になっている場合に、送信されるHostヘッダーにプロキシのホスト名が設定されるというバグがありました。この不適切な動作は、プロキシやオリジンサーバーの誤動作の原因となります。
  - Webクライアントは，すべてのリクエストメッセージにHostヘッダフィールドを含める必要があります。
  - Web プロキシは，リクエストメッセージを転送する前に，Host ヘッダを追加しなければなりません。
  - HTTP/1.1 ウェブサーバは，Host ヘッダーフィールドを持たない HTTP/1.1 リクエストメッセージに対して，400 ステータスコードで応答しなければなりません。

#### 18.2.3.2 Missing Host headers
- 使用されている古いブラウザの中には、Hostヘッダを送信しないものがごく一部あります。バーチャル・ホスティング・サーバがHostヘッダを使ってどのウェブサイトを提供するかを決定していて、Hostヘッダが存在しない場合、おそらくユーザをデフォルトのウェブページ（ISPのウェブページなど）に誘導するか、ユーザにブラウザのアップグレードを勧めるエラーページを返すことになります。

#### 18.2.3.3 Interpreting Host headers
- バーチャルホストではなく、要求されたホストによってリソースを区別することを許可していないオリジンサーバーは、Hostヘッダーフィールドの値を無視しても構いません。しかし、ホストに基づいてリソースを区別するオリジンサーバーは、HTTP/1.1リクエストで要求されたリソースを決定するために、以下のルールを使用しなければなりません。
  1. HTTPリクエストメッセージ中のURLが絶対的なもの(すなわち、スキームとホストのコンポーネントを含む)である場合、Hostヘッダーの値は無視され、URLが優先される。
  2. HTTPリクエストメッセージ中のURLがホストを持たず，リクエストがHostヘッダを含む場合， Hostヘッダからホスト／ポートの値を取得します。
  3. ステップ1または2で有効なホストが見つからない場合は，クライアントに400 Bad Response レスポンスが返される。

#### 18.2.3.4 Host headers and proxies
- ブラウザのバージョンによっては、特にプロキシを使用するように設定されている場合、不正な Host ヘッダを送信するものがあります。例えば、プロキシを使用するように設定されている場合、AppleおよびPointCastクライアントの一部の古いバージョンでは、Hostヘッダにオリジンサーバではなくプロキシの名前を誤って送信してしまいます。

## 18.3 Making Web Sites Reliable
- Webサイトが壊れるのには、いくつかのタイミングがあります。
  - サーバーのダウンタイム
  - トラフィックの急増：突然、誰もが特定のニュース放送を見たいと思ったり、セールに駆けつけたいと思ったりする。突発的なトラフィックの急増は、ウェブサーバに負荷をかけ、速度を低下させたり、完全に停止させたりします。
  - ネットワークの停止や損失
- このセクションでは、これらの一般的な問題を予測し、対処するためのいくつかの方法を紹介します。

### 18.3.1 Mirrored Server Farms

### 18.3.2 Content Distribution Networks
- コンテンツ配信ネットワーク（CDN）とは、特定のコンテンツを配信することを目的としたネットワークのことです。ネットワークのノードには、ウェブサーバー、サロゲート、プロキシキャッシュなどがあります。

### 18.3.3 Surrogate Caches in CDNs
- 図18-6および図18-7では、レプリカオリジンサーバーの代わりにサロゲートキャッシュを使用することができます。サロゲートはリバースプロキシとも呼ばれ、ミラーリングされたウェブサーバと同様に、コンテンツに対するサーバからのリクエストを受け取ります。サロゲートは、特定のオリジンサーバーに代わってサーバーリクエストを受け取ります（これは、コンテンツ用のIPアドレスが広告される方法により可能となります。通常、オリジンサーバーとサロゲートの間には協力関係があり、サロゲートは特定のオリジンサーバーに向けたリクエストを受け取ることを期待しています）。
- サロゲートとミラーリングされたサーバーの違いは、サロゲートが一般的に需要に応じて利用されることです。サロゲートは、オリジンサーバーのコンテンツの全コピーを保存するのではなく、クライアントが要求するコンテンツを保存します。サロゲートのキャッシュに格納されるコンテンツは、サロゲートが受け取るリクエストに依存しています。ホットコンテンツ（需要の高いコンテンツ）へのアクセスを容易にするために、一部のサロゲートは「プリフェッチ」機能を備えており、ユーザーのリクエストに先立ってコンテンツを引き出すことができる。
- サロゲートを使用したCDNでは、キャッシュ階層の可能性という複雑さがあります。
...
- HTTPは単独でWebを歩いているわけではありません。HTTPメッセージのデータは、その旅の途中で多くのプロトコルによって管理されます。しかし、ミラーリングされたサーバー、ウェブプロキシ、キャッシュなどが存在する世界では、HTTP メッセージの宛先は必ずしも単純ではありません。
- 本章では、HTTPメッセージの最終目的地を決定するネットワークツール、技術、プロトコルであるリダイレクション技術について説明します。リダイレクション技術は、通常、メッセージの最終的な送信先を、プロキシ、キャッシュ、またはサーバーファーム内の特定のウェブサーバーのいずれかに決定します。リダイレクション技術は、クライアントが明示的に要求していない場所にメッセージを送信することもあります。
- この章では、以下のリダイレクト技術について、その仕組みと、負荷分散の機能（ある場合）を説明します。
  - HTTP redirection
  - DNS redirection
  - Anycast routing
  - Policy routing
...
## 20.1 Why Redirect?
- 現代のウェブでは、リダイレクションが不可欠です。なぜなら、HTTPアプリケーションは常に次の3つのことを実現したいからです。
  - HTTPトランザクションを確実に実行する
  - 遅延の最小化
  - ネットワークの帯域を節約する
- このような理由から、ウェブコンテンツは複数の場所に分散されることが多くなっています。これは、ある場所が故障しても別の場所が利用できるように、信頼性を高めるために行われています。また、クライアントがより近いリソースにアクセスできれば、要求されたコンテンツをより早く受け取ることができるため、応答時間を短縮するために行われています。さらに、ターゲットとなるサーバーを分散させることで、ネットワークの混雑を緩和するために行われています。リダイレクトは、分散されたコンテンツの「最適」を見つけるための一連の技術と考えることができます。
- リダイレクションとロードバランシングは共存するものなので、ロードバランシングの話題も含まれています。ほとんどのリダイレクションでは、何らかの形で負荷分散が行われています。つまり、受信したメッセージの負荷を一連のサーバーに分散することができます。逆に言えば、負荷分散にはリダイレクションも含まれます。なぜなら、負荷を共有するサーバー間で着信メッセージを何らかの方法で分散させる必要があるからです。

## 20.2 Where to Redirect
- サーバー、プロキシ、キャッシュ、ゲートウェイは、クライアントがHTTPリクエストを送信すると、それを処理するという意味で、クライアントからはすべてサーバーとして見えます。多くのリダイレクト技術は、サーバー、プロキシ、キャッシュ、ゲートウェイに共通するサーバーのような特徴を持っているため、それらに対して機能します。その他のリダイレクション技術は、特定のクラスのエンドポイント用に特別に設計されており、一般的には適用できません。この章の後のセクションでは、一般的なテクニックと特殊なテクニックを紹介します。
- WebサーバーはIPごとにリクエストを処理します。重複するサーバーにリクエストを分配するということは、特定のURLに対する各リクエストを、最適なWebサーバー（クライアントに最も近いサーバー、最も負荷の少ないサーバー、その他最適なもの）に送ることを意味します。サーバーへのリダイレクトは、ガソリンを求めるすべてのドライバーを最寄りのガソリンスタンドに送るようなものです。
- プロキシは、プロトコルごとにリクエストを処理する傾向があります。理想的には、プロキシの近隣にあるすべての HTTP トラフィックはプロキシを経由するべきです。たとえば、プロキシキャッシュがさまざまなクライアントの近くにある場合、すべてのリクエストはプロキシキャッシュを経由するのが理想的です。なぜなら、キャッシュは人気のあるドキュメントを保存し、それらを直接提供するので、オリジンサーバへの長くて高価な移動を避けることができるからです。プロキシにリダイレクトすることは、主要なアクセス道路の交通量を（それがどこに向かっているかに関わらず）ローカルな近道に吸い上げるようなものです。

## 20.3 Overview of Redirection Protocols
- リダイレクションの目的は、HTTPメッセージを利用可能なWebサーバーにできるだけ早く送ることです。HTTPメッセージがインターネット上でどのような方向に進むかは、HTTPアプリケーションやルーティングデバイスの影響を受けます。例えば、以下のようになります。
  - クライアントのメッセージを作成するブラウザアプリケーションは、プロキシサーバーに送信するように設定することができます。
  - DNSリゾルバは、メッセージのアドレスに使用されるIPアドレスを選択します。このIPアドレスは、地理的に異なる場所にいるクライアントごとに異なることがあります。
  - スイッチやルーターは、パケットのTCP/IPアドレスを調べ、それに基づいてパケットのルーティングを決定します。
  - Webサーバーは、HTTPリダイレクトにより、リクエストを別のWebサーバーに送り返すことができます。
- ブラウザの設定、DNS、TCP/IP ルーティング、HTTP はすべて、メッセージをリダイレクトするメカニズムを備えています。ブラウザの設定のように、トラフィックをプロキシにリダイレクトするためだけに意味がある方法もあれば、DNSリダイレクトのように、トラフィックを任意のサーバに送るために使用できる方法もあることに注意してください。
- 表20-1は、メッセージをサーバーにリダイレクトするためのリダイレクト方法をまとめたもので、それぞれの方法についてはこの章で後述します。

| Mechanism | How it works | Basis for rerouting | Limitations |
| ---- | ---- | ---- | ---- |
| HTTP redirection | 最初のHTTPリクエストは、コンテンツを提供する「最適な」ウェブサーバを選択する第1のウェブサーバに送られます。最初のウェブサーバは、選択されたサーバへのHTTPリダイレクトをクライアントに送信します。クライアントは、選択されたサーバーにリクエストを再送信します。 | ラウンドロビン方式のロードバランシング、レイテンシーの最小化、最短経路の選択など、多くの選択肢があります。 | すべてのトランザクションに余分なリダイレクト処理が必要なため、時間がかかることがあります。また、最初のサーバーがリクエストの負荷に対応できなければなりません。 |
| DNS redirection | DNSサーバーは、URLに含まれるホスト名に対して、複数のIPアドレスの中からどのIPアドレスを返すかを決定します。 | ラウンドロビン方式のロードバランシング、レイテンシーの最小化、最短経路の選択など、多くの選択肢があります。 | DNSサーバーを設定する必要がある。 |
| Anycast addressing | 複数のサーバーが同じIPアドレスを使用している。各サーバは、バックボーンルータになりすまします。他のルーターは、共有IPアドレス宛のパケットを、最も近いサーバーに送信します（最も近いルーターにパケットを送信していると思い込みます）。 | ルーターには、内蔵された最短経路のルーティング機能があります。 | ルーターを所有/設定する必要がある。アドレスの競合のリスクがある。ルーティングが変更され、接続に関連するパケットが異なるサーバーに送信されると、確立されたTCP接続が切れる可能性がある。 |
| IP MAC forwarding | スイッチやルーターなどのネットワーク機器は、パケットの宛先アドレスを読み取ります。そのパケットがリダイレクトされるべきものであれば、スイッチはそのパケットにサーバーやプロキシの宛先MACアドレスを与えます。 | 帯域の節約とQOSの向上 ロードバランス。 | サーバーやプロキシは1ホップ離れている必要があります。 |
| IP address forwarding | レイヤ4スイッチは、パケットの宛先ポートを評価し、リダイレクトパケットのIPアドレスをプロキシやミラーリングされたサーバのものに変更します。 | 帯域の節約とQOSの向上 ロードバランス。 | クライアントのIPアドレスがサーバー/プロキシに筒抜けになることがあります。 |

- プロキシサーバーにメッセージをリダイレクトするためのリダイレクト方法を表20-2にまとめました。

| Mechanism | How it works | Basis for rerouting | Limitations |
| ---- | ---- | ---- | ---- |
| Explicit browser configuration | Webブラウザは、HTTPメッセージを近くのプロキシ（通常はキャッシュ）に送信するように設定されています。この設定は、エンドユーザーが行うことも、ブラウザを管理するサービスが行うこともできます。 | 帯域幅を節約し、QOSを向上させる。ロードバランス。 | ブラウザの設定能力に依存する。 |
| Proxy autoconfiguration (PAC)  | Webブラウザが設定サーバからPACファイルを取得する。PACファイルは、各URLに対してどのプロキシを使用するかをブラウザに伝えます。 | 帯域を節約し、QOSを向上させる。ロードバランス。 | ブラウザが設定サーバに問い合わせるように設定されている必要があります。 |
| Web ProxyAutodiscovery Protocol (WPAD) | Webブラウザが設定サーバにPACファイルのURLを問い合わせる。PAC単独の場合とは異なり、ブラウザは特定の構成サーバで構成されている必要はありません。 | 設定サーバは、クライアントのHTTPリクエストヘッダの情報をもとにURLを決定する。ロードバランス。 | WPADをサポートしているブラウザは限られています。 |
| Web Cache Coordination Protocol (WCCP) | ルーターは、パケットの宛先アドレスを評価し、リダイレクトパケットをプロキシまたはミラーリングされたサーバーのIPアドレスでカプセル化します。既存の多くのルーターで動作します。パケットをカプセル化できるので、クライアントのIPアドレスが失われることはありません。 | 帯域の節約とQOSの向上を実現。ロードバランス。 | WCCPに対応したルーターを使用する必要がある。トポロジー上の制限があります。 |
| Internet Cache Protocol (ICP) | プロキシキャッシュは、要求されたコンテンツを兄弟キャッシュのグループに問い合わせることができます。また、キャッシュの階層化にも対応しています。 | オリジンサーバーに問い合わせるよりも、兄弟キャッシュや親キャッシュからコンテンツを取得する方が高速です。 | URLのみでコンテンツを要求するため、誤ったキャッシュがヒットすることがあります。 |
| Cache Array Routing Protocol (CARP) | プロキシキャッシュハッシュプロトコル。キャッシュがリクエストを親キャッシュに転送することを可能にします。ICPとは異なり、キャッシュ上のコンテンツは分離されており、キャッシュ群は1つの大きなキャッシュとして機能します。 | 近くのピア・キャッシュからコンテンツを取得する方が、オリジン・サーバーに申請するよりも高速です。 | CARPは兄弟関係をサポートできません。全てのCARPクライアントは設定に同意しなければなりません。そうしないと、異なるクライアントが同じURIを異なる親に送ることになり、ヒット率が低下します。 |
| Hyper Text Caching Protocol (HTCP) | 参加しているプロキシキャッシュは、要求されたコンテンツを兄弟キャッシュのグループに問い合わせることができます。HTTP 1.0 および 1.1 ヘッダーをサポートしており、キャッシュのクエリを微調整することができます。 | オリジンサーバーに申請するよりも、兄弟キャッシュや親キャッシュからコンテンツを取得する方が速い。 |  |

## 20.4 General Redirection Methods
- このセクションでは、サーバとプロキシの両方で一般的に使用されている様々なリダイレクト手法について深く掘り下げていきます。これらの技術は、トラフィックを別の（おそらくより最適な）サーバにリダイレクトしたり、プロキシを介してトラフィックをベクトル化したりするために使用できます。具体的には、HTTPリダイレクション、DNSリダイレクション、エニーキャストアドレッシング、IP MACフォワーディング、IPアドレスフォワーディングについて説明します。
...
- ほとんどのサーバやプロキシは、処理したHTTPトランザクションの概要を記録します。これは、使用状況の追跡、セキュリティ、課金、エラー検出など、さまざまな理由で行われます。この章では、HTTP トランザクションに関するどのような情報がログに記録されるのか、また一般的なログフォーマットにはどのようなものがあるのかなど、ログについて簡単にご紹介します。
