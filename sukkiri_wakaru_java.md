...
- クラスは特に理由がない限りpublicで修飾するのが一般的

## getterとsetter
- getter: フィールドの中身を呼び出し元に返すだけのメソッド
- getterメソッドの定石
```
...
2. フィールドの名前など、クラスの内部設計を自由に変更できる
  - フィールドの名前変更であれば、getterやsetterからの参照先フィールド名を変えるだけでよい
3. フィールドへのアクセスを検査できる
  - setterで設定されようとしている値が妥当かどうか検査するのが定石

## クラスに対するアクセス制御
- クラスへのアクセス制御　2種類
package private 記述：何も書かない 自分と同じパッケージに属するクラス
public 記述：public すべてのクラス

- 非publicクラスの特徴
  1. クラスの名前はソースファイル名と異なってもよい。
  2. 1つのソースファイルに複数宣言してもよい。
...
- 複数のクラスを親として1つのクラスを定義する多重継承はJavaではできない。

- 親クラスを継承して子クラスを宣言する際に、親クラスのメンバを子クラス側で上書きすることをオーバーライドという。
- 継承を用いて子クラスに宣言されたメンバ
  1. 親クラスに同じメンバがなければ、そのメンバは「追加」になる。
  2. 親クラスに同じメンバがあれば、そのメンバは「上書き変更」される。

- 継承やオーバーライドの禁止
  - Javaでは、**宣言時にfinalが付けられているクラスは継承できない。**
  - また、**宣言にfinalが付けられたメソッドは、子クラスでオーバーライドできない。**

## インスタンスの姿
- 継承されたクラスから生成されたインスタンスは、外から見るとその子インスタンスだが、内部に親クラスのインスタンスを含んでおり、二重構造になっている。
- インスタンスの外部からメソッドの呼び出しがあった場合、**多重構造のインスタンスは、極力、外側にある子インスタンス部分のメソッドで対応しようとする。**（子クラスでオーバーライドしたメソッドがあった場合、親クラスの当該メソッドもインスタンス内にあるが、呼び出しが届かないようになっている。）
- 親インスタンス部のフィールド、メソッドを利用する場合
```
super.フィールド名
super.メソッド名(引数)
```
...
- Javaでは、**「すべてのコンストラクタは、その先頭で必ず内部インスタンス部(=親クラス)のコンストラクタを呼び出さなければならない」**というルールになっている。
- 親クラスのコンストラクタの呼び出し
```
super(引数);
* ただし、コンストラクタの最初の行にしか記述できない。
```
- **プログラマがコンストラクタの1行目にsuper()を書いていない場合、コンパイラによってsuper();という行が自動的に挿入される。**

## 正しい継承、間違った継承
- **正しい継承とは、「is-aの原則」といわれるルールに則っている継承**のことである。
- is-aの関係
  - 子クラス is-a 親クラス（**子クラスは、親クラスの一種である**）
- is-aの原則が成立しないなら、継承は使ってはいけない。
- 継承は、「ある2つのクラスに特化・汎化の関係があることを示す」ための道具でもある。

# 第12章 高度な継承
- 本章では、継承ツリーの上部に登場する抽象的なクラスの定義方法を学ぶ。
- 抽象的なクラスは、「他の開発者が効率よく安心して利用できる継承の材料を作る」という意識で作成する。

## 抽象的なクラスの問題点
- 抽象的なクラスを作ろうとしたときに下記のような問題が生じる
  1. 詳細未定メソッドの存在による、子クラスでのオーバーライド忘れ
  2. 詳細未定メソッドの「何もしない」メソッドと区別がつかない
  3. 抽象的なクラスを意図せずnewできてしまう

## 抽象クラス
- 詳細未定メソッド（抽象メソッド）の宣言
  - 下記の書き方は、「メソッドは少なくとも宣言すべきだが、具体的な内容を確定できないので記載しない」という表明になる。そのため、問題点2を解決できる。
```
...
- 抽象メソッドを含むクラスの宣言
  - 抽象メソッドを持つクラス宣言時は、下記のようにしなければならない。
  - このようなクラスを**抽象クラス**といい、newによるインスタンス化が禁止される。（問題3の解決）
...
- 抽象メソッドがあるクラスを継承する際は、そのメソッドをオーバーライドしないと、子クラスのnew時にコンパイルエラーが出る。（問題1は解決される）

## インタフェース
- Javaでは次の条件を満たす「特に抽象度が高い抽象クラス」を、**インタフェース**として特別扱いできる。
...
- インタフェースに宣言されたメソッドは、自動的にpublicかつabstractになるため次のように書ける。
例：
```
...
- また、public static finalが付いたフィールド(定数)だけは宣言できる。その際public static finalは省略できる。
...
#### インタフェースの効果
1. 同じインタフェースを**implementsする複数の子クラスたちに、共通のメソッド群を実装するように強制できる。**
2. あるクラスがインタフェースを実装していれば、**少なくともそのインタフェースが定めたメソッドは持っていることが保証される。**
...
#### インタフェースによる多重継承
```
アクセス修飾子 class クラス名 implements 親インタフェース名1, 親インタフェース名2, ... {
  ...
}
```

- インタフェースを定義するとき、既存のインタフェースをextendsで拡張することもできる。
- 使い分けは下記。
...
- extendsとimplementsの両方を使ったクラス定義も可能
- Java8以降では、defaultキーワードを用いることで、処理のデフォルト実装を添えた抽象メソッドを定義できるようになった。継承先でオーバーライドされなかった場合、デフォルト実装の内容でオーバーライドされたものと見なされる。
```
...
# 第13章 多態性
- 多態性を活用するときは、「箱の型」と「中身の型」が異なるケースが出てくる。あるインスタンスをどのように捉えるか、はどの型の変数に代入するかで決まる。
- Javaでは、箱(変数)と中身(インスタンス)の関係性について、絵に描いてみて嘘にならないような代入は許される。
- ただし、Javaの継承関係で変数とインスタンスの型が繋がっていなければ代入できない。
- 抽象クラスやインタフェースからインスタンスを生み出すことはできないが、型を利用することは可能。

#### 呼び出せるメソッドの変化、メソッドを呼び出せた場合の処理について
- 変数の型が抽象クラスで具象クラスがインスタンスとして格納されている場合、その変数に対して具象クラスのメソッドを呼べず、抽象クラスのメソッドしか呼べない。
- 実際に動くメソッドの中身はインスタンスの型(中身の型)によって決まる。（= 変数の型のメソッドでなく、インスタンスの型のメソッドが呼ばれる。）どんな型の変数に入っているかは関係ない。
  - 例：Monster <- Slime という親子関係があった場合　Monster型の変数にSlime型のインスタンスを代入して、その変数に対してrun()メソッドを呼んだら、Slime型でオーバーライドしているrun()メソッドが呼ばれる。
...
## 捉え方を変更する方法
- 「あいまいな型に入っている中身を厳密な型に代入する」キャストはダウンキャストといい、失敗の危険が伴う。
例：
```
Character c = new Wizard();
Wizard w = (Wizard) c;
```
- 型変換で嘘の構図になる場合、コンパイルは成功するが実行時にClassCastExceptionエラーが出る。
- 安全にキャストできるかを判定するには下記のようinstanceof演算子を使う。
```
変数 instanceof 型名
* 変数を型名の箱に代入可能ならばtrueが返る。
```
...
## 多態性のメリット
- 呼び出し側は相手を同一視し、同じように呼び出すのに、呼び出される側は、きちんと自分に決められた動き(自分のクラスでオーバーライドされたメソッド内の処理)をする（同じ呼び出しなのに、多数の異なる状態を生み出すことがある）という特性から「多態性」という名前が付けられている。
